...
class FireSentry5000Builder:
    ...
    def build_feature_matrix_with_progress(self, fire_points, pseudo_points):
        ...
        # Reconstruct metadata columns from original points (needed for temporal split)
        feature_matrix = results_df[feature_names + ['is_fire']].copy()
        
        # Add metadata columns from original points using idx mapping
        # The results_df index is the idx from the worker, which matches all_points index
        metadata_cols = ['date', 'lat', 'lon', 'dtw_start', 'dtw_end']
        for col in metadata_cols:
            if col in all_points.columns:
                # Map using index (idx) from results to get corresponding metadata from all_points
                feature_matrix[col] = feature_matrix.index.map(
                    lambda idx: all_points.loc[idx, col] if idx in all_points.index else (pd.NaT if 'date' in col or 'dtw' in col else np.nan)
                )
    ...
    def evaluate_model(self):
        ...
        try:
            ...
            if 'date' not in feature_data.columns:
                logger.error("❌ 'date' column not found in feature matrix!")
                logger.error("Feature matrix needs to be rebuilt with date column included.")
                logger.error("Please run: python scripts/build_features_5000.py --force")
                return False
